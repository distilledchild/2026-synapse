# Synapse 0.2.10 — Local Messages app

For Apple Silicon Macs. Built for macOS 13 or later; tested on macOS 26.6.2.

## Install and connect

1. Open `Synapse-0.2.10-arm64.dmg` and drag Synapse into Applications. Quit the existing app before replacing it.
...
239 automated checks passed: 44 database/body checks, 54 text-send checks, 31 Contacts checks, 29 Messages-name checks, 21 history checks, 21 composer/image checks, 24 photo-send checks, and 15 status-bar layout checks.
...
Unzip `Synapse-0.2.10-source.zip` and run from its root:
